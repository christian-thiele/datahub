...
ApiRequest _request(String url) {
  ...
  return ApiRequest(uri, HttpRequestMethod.get, {}, {}, Stream.empty());
}
...
void _queryString() {
  ...
  expect(
    _request('http://localhost/?abc=hi%20there').getParam('abc'),
    'hi there',
  );
  expect(_request('http://localhost/?abc=123').getParam<String?>('abd'), null);
  expect(
    () => _request('http://localhost/?abc=123').getParam<String>('abd'),
    throwsException,
  );
}
...
void _queryInt() {
  ...
  expect(
    () => _request('http://localhost/?abc=123').getParam<int>('abd'),
    throwsException,
  );
}
...
void _queryDateTime() {
  ...
  expect(
    _request(
      'http://localhost/?timestamp=1688546106000',
    ).getParam<DateTime>('timestamp').toUtc(),
...
void _queryIntList() {
  ...
  expect(
    _request(
      'http://localhost/?list=2&list=4&list=6&other=8',
