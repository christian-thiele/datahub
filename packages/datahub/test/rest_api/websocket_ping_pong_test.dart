...
void main() {
  declareTest('Websocket Ping-Pong', [
    ApiService(
      routes: [
        WebsocketEndpoint(
          onSession: (session) {
    ...
    expect(pings.length, greaterThanOrEqualTo(2), reason: 'Should have received at least 2 pings and responded to them');
    expect(doneCompleter.isCompleted, isFalse, reason: 'Connection should still be open because we responded to pings');

    // Now stop responding and wait for timeout
    await doneCompleter.future.timeout(const Duration(seconds: 2));

    expect(pings.length, greaterThan(2), reason: 'Should have received more pings before timeout');
    expect(doneCompleter.isCompleted, isTrue, reason: 'Connection should be closed after heartbeat timeout');
