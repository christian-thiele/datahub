...
abstract interface class $ExampleObject with DataObject<ExampleObject> {
  const $ExampleObject();
  ...
  static final DataBean<ExampleObject> bean = DataBean<ExampleObject>(
    name: 'ExampleObject',
    fields: List<DataField<ExampleObject, dynamic>>.unmodifiable([$slideshow]),
  ...
  ExampleObject copyWith({Slideshow? slideshow}) {
    final $data = this as ExampleObject;
    return ExampleObject(slideshow: slideshow ?? $data.slideshow);
  }

  static ExampleObject fromValues(Map<String, dynamic> data) {
    return ExampleObject(slideshow: data['slideshow']);
  }
  ...
  Map<String, dynamic> toJson() {
    final $$data = this as ExampleObject;
    return {'slideshow': $slideshow.toJson($$data.slideshow)}
