---
title: DataHub Aperture
---

<p align="center">
<img src="https://datahubproject.net/logo_shadow.svg" />
</p>

<h2 align="center">DataHub Aperture</h2>
<p align="center">
This library is part of the DataHub Project.<br/>
<a href="https://datahubproject.net">https://datahubproject.net</a>
</p>

![Pub Version](https://img.shields.io/pub/v/datahub_aperture?color=2CB7F6&label=pub.dev&logo=dart&style=flat-square)
![Pub Likes](https://img.shields.io/pub/likes/datahub_aperture?color=2CB7F6&label=pub.dev%20likes&style=flat-square)

> DataHub is a Cloud Development Ecosystem aiming to bring the power of Dart into the Cloud.

*DataHub is still under development and is not to be considered production ready. Comprehensive documentation is yet to
be released.*

---

### Features

This library provides an easy to use administration front-end for DataHub services.

### Usage

Add `datahub_aperture` to the `dependencies` section of your projects
`pubspec.yaml` file, or simply run:

```shell
$ dart pub add datahub_aperture
```


[1]: https://github.com/christian-thiele/datahub